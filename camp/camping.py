...
def get_num_available_sites(
    ...
    for site, availabilities in park_information.items():
        desired_available = [date for date in availabilities if date in dates]
...
def consecutive_nights(available, nights):
    ordinal_dates = [
        datetime.strptime(
            dstr, DateFormat.ISO_DATE_FORMAT_RESPONSE.value).toordinal()
        for dstr in available
    ]
    c = count()
    consecutive_ranges = [list(g) for _, g in groupby(
        ordinal_dates, lambda x: x - next(c))]
...
def run_campsite_check(parks, start_date, end_date, campsite_type=None, campsite_ids=(), nights=None, weekends_only=False, excluded_site_ids=[], json_output=False):
    info_by_park_id = {}
    for park_id in parks:
        info_by_park_id[park_id] = check_park(
            park_id, start_date, end_date, campsite_type, campsite_ids, nights=nights, weekends_only=weekends_only, excluded_site_ids=excluded_site_ids
        )
    if json_output:
